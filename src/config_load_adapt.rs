...
pub fn is_split_section_file<S>(
...
fn adapt_split_section_body(
    ...
    match value {
        YamlValue::Sequence(sequence) => wrap_inner_field(YamlValue::Sequence(sequence), inner_field),
        YamlValue::Mapping(map) if map.contains_key(YamlValue::String(inner_field_name.clone())) => {
            YamlValue::Mapping(map)
        }
        other => other,
    }
...
fn adapt_section_value(value: YamlValue, context: &TransparentSectionContext, section: &str) -> YamlValue {
