...
`generate-template --output <file-name>`은 `config/<root_config_name>/` 아래에
...
`generate-schema --output <path>`는 루트 Draft 7 JSON Schema와 섹션 스키마를
씁니다. 출력 경로를 제공하지 않으면 루트 스키마는
`config/<root_config_name>/<root_config_name>.schema.json`에 쓰입니다.

`validate-config`는 전체 런타임 설정 트리를 로드하고 `confique` 기본값과 검증을
