...
模板由运行时使用的同一个 `confique` schema(结构定义) 生成。生成器会渲染文档注释、默认值提示、必填字段说明和声明的环境变量名；带默认值的字段在 YAML、TOML 和 JSON5 模板中会写出可直接加载的未注释赋值。
...
`load_config` 和 `validate-config` 会执行这类运行时校验。
...
运行时与模板生成均通过 schema 中的 `include` 字段声明子配置文件。TOML 示例：`include = ["server.toml"]`；JSON5 示例：`include: ["server.json"]`。

模板 API 会严格写入调用方传入的 `output_path`。内置的 `generate-template`
